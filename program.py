...
def standardize_rhyme_form(word, allow_vowel_substitution=True):
    ...
    # Normalize common Middle English endings:
    # Match 'en', 'yn', 'in', 'on', or 'un' at the end of the word, and replace with 'en'
    word = re.sub(r'(en|yn|in|on|un)$', 'en', word)

    # Match 'ie', 'i', or 'y' at the end of the word, and replace with 'y'
    word = re.sub(r'(ie|i|y)$', 'y', word)

    # Match 'our', 'or', 'ur', or 'er' at the end of the word, and replace with 'ur'
    word = re.sub(r'(our|or|ur|er)$', 'ur', word)

    # If vowel substitution is allowed:
    if allow_vowel_substitution:
        # Match 'ai', 'ay', 'ei', or 'ey' at the end of the word, and replace with 'ay'
        word = re.sub(r'(ai|ay|ei|ey)$', 'ay', word)
        # Match a vowel followed by 'wr' or 'w' at the end of the word, replace with vowel + 'ur'
...
def break_into_syllables(word):
    ...
    # Replace common vowel combinations with an underscore to treat them as single units
    ...
    # Replace common consonant blends with an underscore to treat them as single units
    for blend in ["sh", "ch", "st", "cl", "fl", "gl", "bl", "wh", "gh", "th", "ll"]:
        word = word.replace(blend, blend + "_")

    # Use regex to find syllables by identifying vowel clusters and separating them
    syllables = re.findall(r'[^aeiouy]*[aeiouy]+(?:[^aeiouy]*|$)', word)

    # Restore replaced placeholders (underscores) from the syllables
...
def ends_in_common_feminine(word1, word2):
    for ending in feminine_endings:
        # Check if both words end with the same known feminine ending
